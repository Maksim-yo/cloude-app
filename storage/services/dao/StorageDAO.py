...
from storage.exceptions import NonExistingDirectoryError, ObjectExistError, MinioObjectExistError, NotCorrectTypeError
...
class StorageDAO:
    ...
    def is_object_exist(self, user_id: int, item_hash: str):
    ...
    def is_object_existp(self, user_id: int, path: str):
        user = User.objects.get(pk=user_id)
        try:
    ...
    def is_object_exist_errs(self, user_id: int, path: str):
        res = self.is_object_existp(user_id, path)
        if res:
            raise ObjectExistError()

    def _get_object_u(self, user: User, item_hash: str) -> StorageItem:
        item = user.storage_items.get(hash__exact=item_hash)
        if item is None:
            raise NonExistingDirectoryError()
        return item

    def _get_object(self, user_id: int, item_hash: str) -> StorageItem:
        user = User.objects.get(pk=user_id)
        return self._get_object_u(user, item_hash)
    ...
    def get_root(self, user_id: int) -> StorageDTO:
        user = User.objects.get(pk=user_id)
        return self._orm_to_entity(user.root_item)

    def get_object_by_path(self, user_id: int, item_path):
        user = User.objects.get(pk=user_id)
    ...
    def get_root_object(self, user_id: int):
        user = User.objects.get(pk=user_id)
        item = user.root_item
        return self._orm_to_entity(item)
    ...
    def _delete_object_recursive(self, user_id: int, item_hash: str) -> List[StorageDTO]:

        object: StorageItem = self._get_object(user_id, item_hash)
        objects: List[List[StorageItem]] = []
        self._list_objects_recursive(objects, object)
        objects.append([object])
        deleted_objects: List[StorageDTO] = []
        for level in objects:
            for _obj in level:
                deleted_objects.append(self._orm_to_entity(_obj))
                _obj.delete()
        return deleted_objects

    def delete_object(self, user_id: int, item_hash: str, recursive: bool = True) -> List[StorageDTO]:
        try:
            return self._delete_object_recursive(user_id, item_hash)
